...
from libros.models import LibrosRequest, LibrosPrestados, AdminsBibliotecaCompartida
from grupos.models import RequestInvitacion, UsuariosGrupo


def admins_bcompartidas(request):
    context = {}
    if request.user.is_authenticated():
        perfil, created = Perfil.objects.get_or_create(usuario=request.user)
        context['perfil_usuario'] = perfil

        bcompartida_administrada = AdminsBibliotecaCompartida.objects.filter(
            perfil=perfil).select_related('biblioteca_compartida').first()
        context['bcompartida_administrada'] = bcompartida_administrada
...
def notificaciones(request):
    ...
    if request.user.is_authenticated():
        ...
        requests_libros = LibrosRequest.objects.filter(perfil_recepcion=perfil_usuario, aceptado=False,
                                                       eliminado=False).count()

        # Notificacion si tiene libros prestados
        libros_recibidos = LibrosPrestados.objects.filter(perfil_receptor=perfil_usuario,
        ...
        if UsuariosGrupo.objects.filter(perfil=perfil_usuario, es_admin=True).exists():
            usuarios_grupo_obj = UsuariosGrupo.objects.filter(perfil=perfil_usuario, es_admin=True).select_related(
                'grupo')
