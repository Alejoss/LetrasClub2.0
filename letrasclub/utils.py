...
def obtener_historial_libros(perfil):
    ...
    return {'libros_recibidos_usuario': libros_recibidos_usuario,
            'libros_recibidos_bcompartida': libros_recibidos_bcompartida,
...
def mail_pedir_libro(request_libro, mensaje):
    ...
    html_message = render_to_string("pedir_libro_mail.html",
                                    {'usuario_receptor_mail': request_libro.perfil_recepcion.usuario.username,
                                     'nombre_usuario_envio': request_libro.perfil_envio.usuario.username,
                                     'mensaje': mensaje,
                                     'titulo_libro': request_libro.libro.titulo,
                                     'autor_libro': request_libro.libro.autor,
...
def mail_pedir_libro_bcompartida(request_libro, biblioteca_compartida):
	...
	escrito por el usuario NO ESTA EN USO
	"""

    titulo = "%s ha pedido un libro a %s" % (request_libro.perfil_envio.usuario.username, biblioteca_compartida.nombre)
    mensaje_texto = (
        "%s te ha pedido que le prestes el libro %s, por favor visita la biblioteca compartida %s en Letras.Club"
        % (request_libro.perfil_envio, request_libro.libro_disponible.libro.titulo, biblioteca_compartida.nombre))
    html_message = render_to_string("pedir_libro_bcompartida_mail.html",
                                    # !!! perfil_admin no existe
                                    {'usuario_receptor_mail': biblioteca_compartida.perfil_admin.usuario.username,
                                     'nombre_usuario_envio': request_libro.perfil_envio.usuario.username,
                                     'biblioteca_compartida_nombre': biblioteca_compartida.nombre,
                                     'titulo_libro': request_libro.libro_disponible.libro.titulo,
                                     'autor_libro': request_libro.libro_disponible.libro.autor,
                                     'telefono': request_libro.perfil_envio.numero_telefono_contacto})
...
def mail_anunciar_devolucion(libro_prestado):
    ...
    mensaje_texto = ("%s ha marcado tu libro %s como devuelto, si ya lo tienes,\
	 por favor visita tu biblioteca en Letras.Club") % (
        libro_prestado.perfil_receptor.usuario.username, libro_prestado.libro.titulo)
    html_message = render_to_string("anunciar_devolucion_mail.html",
                                    {'usuario_receptor_mail': libro_prestado.perfil_dueno.usuario.username,
                                     'usuario_devolucion': libro_prestado.perfil_receptor.usuario.username,
                                     'titulo_libro': libro_prestado.libro.titulo,
...
def mail_aceptar_prestamo(libro_prestado):
    ...
    titulo = "%s ha aceptado prestarte el libro %s" % (
        libro_prestado.perfil_dueno.usuario.username, libro_prestado.libro.titulo)
    mensaje_texto = "%s ha aceptado prestarte el libro %s de %s, por favor visita tu perfil en Letras.Club"
    html_message = render_to_string("aceptar_prestamo_mail.html",
                                    {'usuario_receptor_mail': libro_prestado.perfil_receptor.usuario.username,
                                     'usuario_dueno_libro': libro_prestado.perfil_dueno.usuario.username,
                                     'titulo_libro': libro_prestado.libro.titulo,
                                     'autor_libro': libro_prestado.libro.autor,
                                     'mensaje': libro_prestado.mensaje_aceptacion})
...
def mail_agregar_a_grupo(invitado_por, usuario_invitado, grupo):
    ...
    html_message = render_to_string("invitar_grupo_mail.html", {'usuario_invitado': usuario_invitado.usuario.username,
                                                                'invitado_por': invitado_por.usuario.username,
...
def mail_request_entrar_grupo(usuario_request, admins, grupo):
    ...
    html_message = render_to_string("request_entrar_grupo_mail.html",
                                    {'usuario_request': usuario_request.usuario.username, 'grupo': grupo.nombre})
...
def enviar_mail_contactanos(nombre, correo, tema, mensaje):
    ...
    receptores = [x[1] for x in settings.ADMINS]
...
def obtener_muro_perfil(perfil):
    ...
    comentarios = CommentPerfil.objects.filter(muro=perfil, eliminado=False).annotate(
        num_respuestas=Count("respuestas"))
    n_excluir = Notificacion.objects.filter(perfil_actor=perfil).exclude(tipo__in=tipos_excluir_query).annotate(
        num_respuestas=Count("respuestas"))
    n_generales = Notificacion.objects.filter(perfil_actor=perfil, grupo=None).annotate(
        num_respuestas=Count("respuestas"))
...
def obtener_muro_bcompartida(biblioteca_compartida):
    comentarios = CommentBCompartida.objects.filter(bcompartida=biblioteca_compartida, eliminado=False).annotate(
        num_respuestas=Count("respuestas"))
    notificaciones = Notificacion.objects.filter(biblioteca_compartida=biblioteca_compartida).annotate(
        num_respuestas=Count("respuestas"))
