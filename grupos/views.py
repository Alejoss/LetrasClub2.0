...
from letrasclub.utils import obtener_perfil, obtener_libros_perfil, obtener_usuario_leyendo, mail_agregar_a_grupo, \
    mail_request_entrar_grupo
...
def invitar(request, id_grupo):
    ...
    for perfil in perfil_obj:
        ...
        lista_usuarios[perfil.usuario.username] = {'imagen_perfil': perfil.imagen_perfil, 'ciudad': ciudad,
...
def aceptar_ajax(request):
    if request.is_ajax():
        ...
        # Crear UsuarioGrupo
        if UsuariosGrupo.objects.filter(perfil=request_invitacion.usuario_invitado, grupo=request_invitacion.grupo,
                                        activo=True).exists():
...
def request_entrar_ajax(request):
    if request.is_ajax():
        ...
        if RequestInvitacion.objects.filter(grupo=grupo, usuario_invitado=perfil_usuario_invitado, aceptado=False,
                                            eliminado=False).exists():
