...
class NManager(models.Manager):
	...
	def presto_libro(self, perfil_presto, perfil_recibio, libro, grupo=None):
		if grupo:
			notificacion = self.create(perfil_actor=perfil_presto, perfil_target=perfil_recibio, libro=libro, grupo=grupo, tipo="presto_libro")
		else:
			notificacion = self.create(perfil_actor=perfil_presto, perfil_target=perfil_recibio, libro=libro, tipo="presto_libro")

		return notificacion

	def bcompartida_presto(self, biblioteca_compartida, perfil_recibio, libro):
		notificacion = self.create(biblioteca_compartida=biblioteca_compartida, perfil_actor=perfil_recibio, libro=libro, tipo="bcompartida_presto")
	...
	def bcompartida_cambio(self, biblioteca_compartida, libro, segundo_libro, usuario_cambiar=None):
		...
		if usuario_cambiar:
			notificacion = self.create(biblioteca_compartida=biblioteca_compartida, perfil_actor=usuario_cambiar, libro=libro, 
				segundo_libro=segundo_libro, tipo="bcompartida_cambio")
		else:
			notificacion = self.create(biblioteca_compartida=biblioteca_compartida, libro=libro, segundo_libro=segundo_libro, tipo="bcompartida_cambio")
...
class Notificacion(models.Model):
	...
	perfil_actor = models.ForeignKey(Perfil, null=True, blank=True, related_name="actor")  # El perfil de la persona que ocasiono la notificación
	perfil_target = models.ForeignKey(Perfil, null=True, blank=True, related_name="target")  # Perfil opcional de segunda persona que debe recibir notificación
